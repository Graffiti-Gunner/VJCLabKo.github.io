...
function genRendererSetup(db, i) {
    ...
    rendererSetup.mmd.stagePath = null;
    //song
...
const rendererInit = function () {
    ...
    bt.onclick = () => {
        ...
        if(v){
            rendererSetup.mmd.stagePath = d.options[d.selectedIndex].value;
            console.log(v);
        }
    ...
    stageSl.name = 'stages';
    stageSl.id = 'stageSl';
    stageFiles = ['./assets/models/stage/Court Yard Day/Court Yard Day.pmd'];
    ...
    for(i in stageFiles){
        ...
        sli.value = stageFiles[0];
        sli.appendChild(document.createTextNode(stageFiles[0].replace(reg,"$1")))
        stageSl.appendChild(sli)
    }
